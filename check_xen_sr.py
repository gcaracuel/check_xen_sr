...
# Modified by: Guillermo Caracuel (gcaracuel@gmail.com) to support connections to poolslaves (it will redirect it)
# Contact if bugs, etc.
#
...
#
# Version history:
# - v1.1.3: Now you could try to query slaves it will autoredirect to poolsmaster
...
def main(session, sr_name, warning, critical, performancedata_format):
	...
	try:
		...
		if e.details[0]=='HOST_IS_SLAVE':
			# If it's an SLAVE NODE we got to connect de the poolmaster
			session=XenAPI.Session("https://"+e.details[1])
			session.login_with_password(username, password)
			# Repeat the command
			sr = session.xenapi.SR.get_by_name_label(sr_name)
		else:
			print "CRITICAL: There is a problem to connect to the server", str(e)
			sys.exit(2)
	...
	if sr:
		...
		if performancedata_format == "pnp4nagios":
			performance = performancedata(sr_name,
						humanize_bytes(total_bytes_b, suffix=False, format=performancedata_format),
						humanize_bytes(total_alloc_b,               format=performancedata_format),
						humanize_bytes(warning_b,     suffix=False, format=performancedata_format),
						humanize_bytes(critical_b,    suffix=False, format=performancedata_format),
		...
		info =  "utilization %s%%, size %s, used %s, free %s | %s" % (str(round(used_percent,2)), 
										str(humanize_bytes(total_bytes_b)), 
										str(humanize_bytes(total_alloc_b)), 
										str(humanize_bytes(free_space_b)), 
										performance)
